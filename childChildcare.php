	...
	else
	{
		if(strcmp($_POST['create'],'')!=0)
		{
			...
			else
			{
				...
				$query="SELECT ChildCName FROM ChildinChildcare WHERE ChildCName='$ename'";
				...
				{
					$query="INSERT INTO ChildinChildcare VALUES ('','$ename','$edate','$emem')";
					$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
					echo "Child in childcare added successfully!";
				}
			}
		}
		if(strcmp($_POST['delete'],'')!=0)
		{
			...
			$set = substr("$dset", 0, -1);
			$query="DELETE FROM ChildinChildcare WHERE ChildCID IN ($set)";
			$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
		...
		if(strcmp($_POST['update'],'')!=0)
		{
			...
			if(!isset($_SESSION['errorFlag']))
				header("Location:childChildcareEdit.php");
		}
		if(strcmp($_POST['updatechildC'],'')!=0)
		{
			$id=$_SESSION['uid'];
			unset($_SESSION['uid']);
			$query="SELECT * FROM ChildinChildcare WHERE ChildCID='$id'";
			$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
			...
			$query="SELECT * FROM ChildinChildcare WHERE ChildCName='$ename2'";
			$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
			$count=mysqli_num_rows($result);
			if($count!=0)
			{
			...
			{			
				if(strcmp($_POST['emem'],'')!=0)
					$emem=$_POST['emem'];	
				$query="UPDATE ChildinChildcare SET ChildCName='$ename',ChildCBDay='$edate',Teacher='$emem' WHERE ChildCID='$id'";
				$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
		...
		{
			$query="SELECT * FROM ChildinChildcare";
			$result = mysqli_query($link, $query) or die("Query failed : " . mysqli_error());
			...
			{
				echo  "<form action='childChildcare.php' method='POST'><table id='result'><tr id='result'><th id='result'>Child Id</th><th id='result'>Child Name</th><th>Child Birthday</th><th id='result'>Teache in Charge</th><th id='result'>Select</th></tr>";
				$j=0;
				while ($line=mysqli_fetch_array($result, MYSQL_ASSOC))
				{
					echo "<tr id='result'>";
...
<form method="POST" ACTION="childChildcare.php">
	<table>
		<tr><th><b>Create Child in Childcare Record</b></th><th>(* are required fields)</th></tr>
		<tr>	
		...
		<tr>
			<td><label for="emem"><b>Related Teacher:</b></label></td>
			<td><?php
